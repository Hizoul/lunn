...
def classify(img):
  ...
  identifiedAs = -1
  for b in range(0, 10):
    res = sklearn.metrics.pairwise.pairwise_distances([center[b], img])[0, 0]
    print("result is ", np.absolute(np.linalg.norm(center[b] - img)), sklearn.metrics.pairwise.pairwise_distances([center[b], img]), sklearn.metrics.pairwise.pairwise_distances([center[b], img])[0, 0])
    if currentMin > res:
      currentMin = res
      identifedAs = b
  return identifedAs
...
trainWith(trainIn1, trainOut1, "diffalgo_trainingssetconfusionmatrix.png", "Confusion matrix for training set classification")
testIn = np.genfromtxt('../data/test_in.csv', delimiter=',')
testOut = np.genfromtxt('../data/test_out.csv', delimiter=',')

trainWith(testIn, testOut, "diffalgo_testssetconfusionmatrix.png", "Confusion matrix for test set classification")